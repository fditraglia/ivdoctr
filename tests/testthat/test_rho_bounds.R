...
test_that("candidate2 is vectorized wrt r_TstarU and k", {
  ...
  k_upper <- c(1, 1)
  k_lower <- c(0.1, 0.1)
  expected_answer <- list(min_edge = c(1, 1), max_edge = c(1, 1))
  ans <- candidate2(r_TstarU_lower, r_TstarU_upper, k_lower, k_upper, obs)
  expect_equivalent(expected_answer, ans)
})
...
test_that("candidate3 properly computes zeros of polynomial to get kappas", {
  ...
  test1 <- with(obs, -r_TstarU_upper ^ 2 * r_Tz ^ 2 * (ans$k_roots - r_Ty ^ 2) ^ 3 +
                     ((2 * r_Ty * r_Tz - r_zy * r_Ty ^ 2) * ans$k_roots - 
                        r_Tz * r_Ty ^ 3) ^ 2 * (1 - r_TstarU_upper ^ 2))
  test2 <- with(obs, -r_TstarU_lower ^ 2 * r_Tz ^ 2 * (ans$k_roots - r_Ty ^ 2) ^ 3 +
                     ((2 * r_Ty * r_Tz - r_zy * r_Ty ^ 2) * ans$k_roots - 
                        r_Tz * r_Ty ^ 3) ^ 2 * (1 - r_TstarU_lower ^ 2))
  expect_equal(test1, 0, tol = 10 ^ -4)
  expect_equal(test2, 0, tol = 10 ^ -4)
})
...
test_that("candidate3 properly computes zeros of polynomial to get kappas with
  ...
  k_lower <- c(0.8, 0.8)
  k_upper <- c(0.9, 0.9)
  r_TstarU_lower <- c(-0.8, -0.8)
  r_TstarU_upper <- c(-0.4, -0.4)
  obs <- list(r_Ty = -0.9,
              r_Tz = 0.24,
              r_zy = -0.3)
  ...
  test2 <- with(obs, -r_TstarU_lower ^ 2 * r_Tz ^ 2 * (ans$k_roots - r_Ty ^ 2) ^ 3 +
                     ((2 * r_Ty * r_Tz - r_zy * r_Ty ^ 2) * ans$k_roots - 
                        r_Tz * r_Ty ^ 3) ^ 2 * (1 - r_TstarU_lower ^ 2))
  expect_equal(test1, c(0, 0), tol = 10 ^ -4)
  expect_equal(test2, c(0, 0), tol = 10 ^ -4)
})
